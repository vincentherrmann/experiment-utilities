...
def get_hyperparameters():
    ...
    # environment
    parser.add_argument('--num_envs', default=4, type=int, required=False)
    # model
...
def main(args):
    # print(encode_world_info({'192.168.0.5' : [0]}))
    # eyIxOTIuMTY4LjAuNSI6IFswXX0=
    # -m deepspeed.launcher.launch --world_info=eyIxOTIuMTY4LjAuNSI6IFswXX0= --node_rank=0 --master_addr=192.168.0.5 --master_port=4242
    ...
    model_engine, model_optimizer, _, _ = deepspeed.initialize(args=args,
                                                               model=model,
    ...
    policy_parameters = filter(lambda p: p.requires_grad, model.parameters())

    policy_engine, policy_optimizer, _, _ = deepspeed.initialize(args=args,
                                                                 model=policy,
                                                                 model_parameters=policy_parameters)

    fp16 = policy_engine.fp16_enabled()
    print(f'fp16={fp16}')

    ### MEMORY ################################################################
    memory = GeneralMemory(size=100,
                           artifact_shapes={"actions": env.action})

    @torch.no_grad()
    def act_in_environment(policy):
        pass
